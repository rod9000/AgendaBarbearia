                ...
                <div class="mb-4">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="w-20 h-20 rounded-full overflow-hidden bg-brand-100 flex items-center justify-center flex-shrink-0">
                            <img id="photoPreview" src="{{ $customer->photo ?? '' }}" class="w-full h-full object-cover {{ $customer->photo ? '' : 'hidden' }}">
                            <svg id="photoPlaceholder" class="w-8 h-8 text-brand-400 {{ $customer->photo ? 'hidden' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                        </div>
                        <div class="space-y-2">
                            ...
                            <button type="button" onclick="removePhoto()" class="block text-sm text-rose-500 hover:text-rose-700 font-medium {{ $customer->photo ? '' : 'hidden' }}" id="removePhotoBtn">Remover foto</button>
                        </div>
                    </div>
                    <input type="hidden" name="photo" id="photoInput" value="{{ $customer->photo ?? '' }}">
